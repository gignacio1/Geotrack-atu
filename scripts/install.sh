...
choose_install_mode() {
  ...
  echo -e "${CYAN}╚══════════════════════════════════════════════════════╝${NC}"
  echo ""
  # Quando o script é baixado pelo curl, ele está sozinho em /tmp e não
  # possui o dist do pacote local. Nesse caso, o padrão correto é GitHub.
  if [ -f "${script_dir}/dist/index.cjs" ] && [ -f "${script_dir}/package.json" ]; then
    default_mode=1
  else
    default_mode=2
  fi

  read -rp "Escolha o modo (padrão: ${default_mode}): " INSTALL_MODE
  INSTALL_MODE="${INSTALL_MODE:-${default_mode}}"

  if [ "$INSTALL_MODE" = "2" ]; then
    echo -e "${GREEN}[OK] Modo selecionado: GitHub (com atualização automática)${NC}"
    INSTALL_FROM_GIT=true
  elif [ "$INSTALL_MODE" = "1" ] && [ -f "${script_dir}/dist/index.cjs" ] && [ -f "${script_dir}/package.json" ]; then
    echo -e "${GREEN}[OK] Modo selecionado: arquivo local${NC}"
    INSTALL_FROM_GIT=false
  else
    echo -e "${RED}[ERRO] O modo local exige dist/index.cjs e package.json no mesmo pacote do instalador.${NC}"
    echo -e "${YELLOW}       Para instalar diretamente pelo curl, escolha o modo 2 — GitHub.${NC}"
    exit 1
  fi
}
...
configure_port() {
  ...
  while true; do
    ...
    PORT="${PORT:-3000}"
    # Aceita também o formato acidental "PORT=3000", mas grava apenas o número.
    PORT="${PORT#PORT=}"
    if [[ "$PORT" =~ ^[0-9]+$ ]] && [ "$PORT" -ge 1 ] && [ "$PORT" -le 65535 ]; then
      break
    fi
    echo -e "${RED}[ERRO] Porta inválida. Informe somente um número entre 1 e 65535 (ex.: 3000).${NC}"
    PORT=""
  done
  ENV_FILE="${INSTALL_DIR}/.env"
...
install_from_git() {
  ...
  echo -e "${BLUE}--- Instalando dependências ---${NC}"
  cd "$INSTALL_DIR"
  # O repositório de atualização já contém o build compilado e ofuscado.
  # Ele não contém o código-fonte nem o script "build".
  npm install --omit=dev --silent
  if [ ! -f "${INSTALL_DIR}/dist/index.cjs" ] || [ ! -f "${INSTALL_DIR}/dist/public/index.html" ]; then
    echo -e "${RED}[ERRO] O repositório não contém um build de produção completo.${NC}"
    exit 1
  fi
  cd - > /dev/null

  echo -e "${GREEN}[OK] Repositório clonado e build protegido validado em ${INSTALL_DIR} — atualização automática habilitada!${NC}"
}
